...
# Install files
# ---------------------------------------------------------------------------
mkdir -p "$TARGET_DIR/backends" "$TARGET_DIR/hooks" "$TARGET_DIR/lib" "$TARGET_DIR/config" "$TARGET_DIR/packs"
...
if [ "$HEADLESS" = true ] && command -v python3 >/dev/null 2>&1; then
    ...
    _set_if AGENT_INDICATOR_TMUX "backends.tmux.enabled"
...
# Check for tmux-agent-indicator coexistence
CLAUDE_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}"
if [ "$INSTALL_CLAUDE" = true ] && [ -f "$CLAUDE_DIR/settings.json" ]; then
    if grep -q "tmux-agent-indicator" "$CLAUDE_DIR/settings.json" 2>/dev/null; then
        echo ""
        echo "Note: tmux-agent-indicator hooks also present."
        echo "You may want to remove them to avoid duplicate tmux styling."
        echo "  tmux-agent-indicator: tmux-only (panes, borders, status, animation)"
        echo "  agent-indicator: unified (terminal + tmux + sound + desktop + push)"
    fi
fi
...
Env var overrides still work:
  ...
  export AGENT_INDICATOR_TMUX=on       # tmux pane/border styling (default: auto)
